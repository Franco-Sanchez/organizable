...
List.prototype.deleteList = function() {
  const deleteList = this.parentElement.querySelector(`.js-delete-list-${this.data.listId}`)
  deleteList.addEventListener('click', async (e) => {
    try { 
      ...
      STORE.currentBoard.lists = STORE.currentBoard.lists.filter(list => list.listId !== this.data.listId);
